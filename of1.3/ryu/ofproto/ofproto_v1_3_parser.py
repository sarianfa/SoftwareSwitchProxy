...
class OFPPacketOut(MsgBase):
    ...
    @classmethod
    def _unpack_actions (self,b, length, offset=0):
        """
      ...
      # Use generic action header for unknown type
        ...
        return (offset, actions)
    #@classmethod 
    #def parser (self, raw, datapath, offset=0):
    #   msg =  OFPPacketOut(datapath, 0xffffffff, 1)
    @classmethod
    def parser(cls, datapath, version, msg_type, msg_len, xid, raw):
  ...
  #          print "res type ", a.type
   ...
   #         print "res port ", a.port
           ...
           if remaining <= 0:
              ...
              offset,msg.data = _read(raw, offset, remaining)
    ...
    def parser(cls, datapath, version, msg_type, msg_len, xid, raw):
       ...
       try:
           (msg.cookie, msg.cookie_mask, msg.table_id,
                      ...
                      msg.out_group, msg.flags) = struct.unpack_from(ofproto.OFP_FLOW_MOD_PACK_STR0, raw, offset)
           offset = (ofproto.OFP_FLOW_MOD_SIZE -
                  ofproto.OFP_MATCH_SIZE)
           #offset += ofproto.OFP_FLOW_MOD_0_SIZE

           msg.match = OFPMatch.parser(raw, offset)
           match_length = utils.round_up(msg.match.length, 8)
           #inst_length = (msg.length - (ofproto.OFP_FLOW_MOD_SIZE -
           #                                 ofproto.OFP_MATCH_SIZE +
           #                                 match_length))
           offset += match_length
           print "match_length ",match_length," msg.match.length ",msg.match.length, " offset ",offset, " len(raw) ", len(raw)
           
           instructions = []
           
           while offset < len(raw):
               inst = OFPInstruction.parser(raw, offset)
               instructions.append(inst)
               offset += inst.len
               print "len (wr) ",len(raw)," offset ",offset
